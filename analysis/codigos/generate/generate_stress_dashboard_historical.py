"""
Stress Event Calendar Dashboard — Historical Dataset
=====================================================
Generates an interactive HTML showing all historical stress events across 60
scenarios for each α threshold. Identical structure to the future dashboard:
...
for alpha in ALPHAS:
    pct  = int(alpha * 100)
    path = os.path.join(IN_DIR, f'events_global_historical_alpha{pct:02d}.csv')
    df = pd.read_csv(path)
    sc_data = {}
...
<title>Stress Events Dashboard — Historical · Multi-α</title>
...
<body>
<div class="container">
  <h1>Stress Events — Historical · Multi-α</h1>
  ...
  <div class="filter-row">
    ...
    <input type="number" id="fDurVal" min="0" max="720" value="0" style="width:52px;font-size:12px;color:var(--text2);border:1px solid var(--border);border-radius:4px;padding:1px 4px;background:var(--bg)">
...
cv.addEventListener('mousemove',e=>{{
  ...
  if(di>=1&&di<=365&&si>=1&&si<=60){{
    ...
    if(cs>0){{
      tt.innerHTML=`<strong style="color:var(--text)">Scenario ${{si}}</strong><br><span style="color:var(--text2)">${{MN[m]}} ${{day}} (day ${{di}})</span><br><span style="color:#BA7517;font-weight:500">${{cs.toFixed(4)}}% cost share</span><br><span style="color:var(--text2)">event days ${{evHere[0]}}–${{evHere[1]}} · ${{(evHere[1]-evHere[0])}} days</span>`;
    }} else {{
...
document.getElementById('fDur').addEventListener('input',e=>{{
  ...
  document.getElementById('fDurVal').value=fDurMin;
  applyFilters();
}});
document.getElementById('fDurVal').addEventListener('input',e=>{{
  let v=Math.min(720,Math.max(0,+e.target.value||0));
  fDurMin=v;
...
document.getElementById('db').addEventListener('dblclick',e=>{{
  ...
  if(!row)return;
  window.parent.postMessage({{type:'openSpFlow',evId:+row.dataset.evid,dataset:'historical'}},'*');
}});
...
out = os.path.expanduser('~/Desktop/Bachelor Thesis/analysis/htmls_uso/stress_events_multi_alpha_historical.html')
