"""
Shadow Price and Transmission Flow Map — Future Dataset
========================================================
Generates an interactive hourly map of Europe for each filtered stress event.
For each hour within an event, the map shows:
  - Country fill colour: electricity shadow price (M€/GWh)
  - Arrows on power lines: transmission flow direction and magnitude
  - Line width: fraction of line capacity used (utilisation %)
  - Hover tooltip: shadow price, flow, capacity, congestion dual variable

Run compute_sp_flow_data.py first to generate the required JSON cache.

Inputs:
  ~/Desktop/Bachelor Thesis/CSVs/sp_flow_data_future.json  (pre-computed by compute_sp_flow_data.py)
...
JSON_PATH = '~/Desktop/Bachelor Thesis/CSVs/sp_flow_data_future.json'
...
<title>Shadow Price Electricity — Stress Events</title>
...
<body>
<h1>Shadow Price Electricity + Transmission Flows — Stress Events (Future, α=25%, cs≥0.2, dur≥1d)</h1>
