...
average matrix. Countries are ordered by Ward hierarchical clustering of the
combined correlation matrix (data-driven, no hardcoded zones).
...
Inputs:
  ZEN-garden future output folder  (if cache missing)
  CSVs_and_JSONs/future/features/features_alpha25_future_per_country.csv  (cached)
...
OUT_DIR      = '~/Desktop/Bachelor Thesis/analysis/fotos/future/correlation_matrix'
FEATURES_CSV = '~/Desktop/Bachelor Thesis/CSVs_and_JSONs/future/features/features_alpha25_future_per_country.csv'
EVENTS_CSV   = '~/Desktop/Bachelor Thesis/CSVs_and_JSONs/future/events_global_alpha/events_global_alpha25.csv'
THRESH_CS    = 0.2
THRESH_DUR   = 1.0
...
n_ev  = len(df_ev)
print(f'{n_ev} future events after filtering')
...
order = dend['ivl']   # country codes ordered by Ward dendrogram

# Cluster boxes for combined plot: N_ZONES clusters derived from the same linkage
...
for feat, corr in corr_matrices.items():
    ...
    ax.set_title(f'Correlation matrix — {feat} — future events (n={n_ev})', fontsize=13)
    fig.tight_layout()
...
# Draw boxes around contiguous clusters
